...
3. **Install**
   ...
   - Option B (npm): `npm install re-finer --save-dev`
...
4. **Add the script tag**
   ...
   ```html
   <script src="https://unpkg.com/re-finer/dist/refiner.js"
...
6. **Recommend MCP server setup**
   ...
   - Recommend one of the following approaches:
     - **Universal (supports Claude Code, Cursor, Codex, Windsurf, etc.):**
       See [add-mcp](https://github.com/neondatabase/add-mcp) — run `npx add-mcp` and follow the prompts to add `re-finer-mcp` as an MCP server
     - **Claude Code only:**
       Run `npx re-finer-mcp init` after installing the package
